...
export default function RepoDetailPage() {
  ...
  // Fetch files inside repository
  const fetchRepoFiles = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/files?repo=${encodeURIComponent(repo)}`);
  ...
  const openEditFileEditor = () => {
    ...
    let filenameRelative = selectedFile.key.substring(repo.length + 1);
  ...
  const handleSaveFile = async () => {
    ...
    const fullKey = `${repo}/${fileRelPath}`;

    try {
      const res = await fetch("/api/files/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ key: fullKey, content: editorState.content }),
      });
  ...
  // Navigate folder updates path param
  const navigateToFolder = (folderPath: string) => {
    setEditorState(prev => ({ ...prev, isOpen: false }));
    router.push(`/repos/${encodeURIComponent(repo)}?path=${encodeURIComponent(folderPath)}`);
  };

  // Directory listing calculations relative to repo name
  const getDirectoryItems = () => {
    const folders = new Set<string>();
    const currentFiles: FileItem[] = [];

    const filteredFiles = files.filter(file => {
      const relKey = file.key.substring(repo.length + 1);
      return relKey.toLowerCase().includes(searchQuery.toLowerCase());
    });

    const prefix = `${repo}/${currentPath}`;
            ...
            {loading ? (
              ...
              <>
                ...
                {folderFiles.map(file => {
                  ...
                  const displayPrefix = repo.length + currentPath.length + 1;
